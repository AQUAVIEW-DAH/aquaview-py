...
| `wait(*, poll_interval=2.0, timeout=900.0, max_interval=30.0) -> ExportJob` | Poll until `done`; raises `JobError` on `failed` / `expired` / timeout. The gap between polls grows geometrically from `poll_interval` to `max_interval`, so a long export makes tens of requests, not hundreds. A rate-limited (429) or transient (502/503/504) poll backs off (honoring `Retry-After`) and retries rather than aborting. Each poll is bounded by the remaining deadline. |
